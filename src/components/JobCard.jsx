...
const JobCard = ({ job }) => {
  ...
  useEffect(() => {
    if (applications && job.id) {
      applications.forEach((item) => Number(item.job) === Number(job.id) && setapplied(true));
    }
  }, [applications, job.id]);
  return (
    <Card className="w-[350px]">
      <CardHeader>
        ...
        <CardDescription className="flex items-center gap-2">
          {job.experience === 0 ? (
            <>
              <BriefcaseBusiness /> <span>Fresher</span>
            </>
          ) : (
            <>
              <BriefcaseBusiness /> <span>{job.experience} Years</span>
            </>
          )}
        </CardDescription>
        <CardDescription className="flex items-center gap-2">
